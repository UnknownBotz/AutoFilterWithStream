...
async def next_page(bot, query):
    ...
    try:
        if settings['auto_delete']:
            btn.insert(0, 
                [
                    InlineKeyboardButton("! Quality !", callback_data=f"select_qual#{userid}"),
...
async def select_language(bot, query):
    ...
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        InlineKeyboardButton("Sᴇʟᴇᴄᴛ Yᴏᴜʀ Dᴇꜱɪʀᴇᴅ Quality ↓", callback_data=f"quality#{userid}#unknown")
    ],[
        InlineKeyboardButton("PreDvd", callback_data=f"quality#{userid}#predvd"),
        InlineKeyboardButton("HDTS", callback_data=f"quality#{userid}#hdts"),
        InlineKeyboardButton("HDTC", callback_data=f"quality#{userid}#hdtc")
    ],[
        InlineKeyboardButton("480P", callback_data=f"quality#{userid}#480"),
        InlineKeyboardButton("720P", callback_data=f"quality#{userid}#720")
    ],[
        InlineKeyboardButton("1080P", callback_data=f"quality#{userid}#1080")
    ],[
        InlineKeyboardButton("2160P", callback_data=f"quality#{userid}#2160"),        
    ],[
        InlineKeyboardButton("Gᴏ Bᴀᴄᴋ", callback_data=f"quality#{userid}#home")
    ]]
    try:
        await query.edit_message_reply_markup(
            reply_markup=InlineKeyboardMarkup(btn)
