...
async def next_page(bot, query):
    ...
    btn.insert(0, [
        ...
        InlineKeyboardButton("! Lᴀɴɢᴜᴀɢᴇs !", callback_data=f"select_qual#{userid}")
    ])
    btn.insert(0, [
...
async def qual_check(bot, query):
    _, userid, language = query.data.split("#")
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    if language == "unknown":
...
async def select_qual(bot, query):
    _, userid = query.data.split("#")
    if int(userid) not in [query.from_user.id, 0]:
        return await query.answer(script.ALRT_TXT.format(query.from_user.first_name), show_alert=True)
    btn = [[
        ...
        InlineKeyboardButton("Eɴɢʟɪꜱʜ", callback_data=f"quality#{userid}#480p"),
        InlineKeyboardButton("Tᴀᴍɪʟ", callback_data=f"quality#{userid}#720p"),
        InlineKeyboardButton("Hɪɴᴅɪ", callback_data=f"quality#{userid}#1080p")
    ],[
        InlineKeyboardButton("Kᴀɴɴᴀᴅᴀ", callback_data=f"lang#{userid}#kan"),
        InlineKeyboardButton("Tᴇʟᴜɢᴜ", callback_data=f"lang#{userid}#tel")
    ],[
